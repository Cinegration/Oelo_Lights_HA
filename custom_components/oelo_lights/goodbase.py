...
class OeloLight(LightEntity, RestoreEntity):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if ATTR_RGB_COLOR in kwargs:
            ...
            self._brightness = kwargs.get(ATTR_BRIGHTNESS, 255)  # Use provided brightness or max brightness
            ...
            # adjust brightness in proxy if needed
        if ATTR_BRIGHTNESS in kwargs:
            ...
            if self._effect in pattern_commands:
                # Construct URL for the effect pattern
        ...
        if url:
            # Send the request to control the light if a valid URL was constructed
            # Before sending, adjust the rgb values in the url to compensate for brightness
            brightness_factor = self._brightness / 255
            new_url = self._adjust_colors_in_url(url, brightness_factor)

            await self._send_request(new_url)
            # maybe change url below to new_url if brightness acts off
            self._last_successful_command = url  # Save the last successful command
        else:
            # If no valid action could be determined, use the last successful command or set to white
            if self._last_successful_command:
                ...
                # await self._send_request(self._last_successful_command)
            else:
                _LOGGER.warning("No previous command available, setting default white color for zone %d", self._zone)
                # Default to white light if no previous command exists
                url = (
                    f"http://{self._ip}/setPattern?patternType=custom&num_zones=1&zones={self._zone}"
                ...
                self._last_successful_command = url  # Save the command as the last successful command
