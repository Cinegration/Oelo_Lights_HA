...
class OeloLight(LightEntity, RestoreEntity):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        if self._effect and self._effect in PATTERNS:
            # Apply the effect using the corresponding pattern command
            pattern_command = PATTERNS[self._effect].format(zone=self._zone)
    ...
    async def async_turn_off(self, **kwargs):
        ...
        url = f"http://{self._ip}/{PATTERNS['Off'].format(zone=self._zone)}"
